...
if($tipo_id != ''){ //verificamos nuevamente que sea una opcion valida
  ...
  $sql = "select idgestion, nombre from tipos where (categoria = '$tipo_id' AND area='$valor_idarea') OR (categoria = 'Todos' AND area='Todos')";  
...
<option class="f-box" value="<?= $op['idgestion'] ?>"><?= $op['nombre'] ?></option>
<?php endforeach; ?>