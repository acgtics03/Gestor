...
// Definir zona horaria
	...
	if($sessionTTL > $inactividad){
        ...
        if(!empty($email)){  
            ...
            $consulta = "SELECT * FROM asistencia WHERE user = '$email' AND fregistro ='$freg' AND Estado='INGRESO'";
            ...
            $mostrar3 = mysqli_num_rows($result);
                 ...
                 if($mostrar3 > 0)  {
                        ...
                        $consulIniRefri = mysqli_query($conection, "SELECT * FROM Parametros WHERE iparametro='2'");
                        $mostrar = mysqli_fetch_assoc($consulIniRefri);
                        $consulFinRefri = mysqli_query($conection, "SELECT * FROM Parametros WHERE iparametro='3'");
                        $mostrar2 = mysqli_fetch_assoc($consulFinRefri);
                        
                        $HoraIniRefri = $mostrar['valorp_hora'];
                        $HoraFinRefri = $mostrar2['valorp_hora'];
                        
                        $HoraIni = $HoraFinRefri;
                        $HoraFin = $HoraIni;
                        
                        if($Hora>$HoraFinRefri){
                        
                        $sql3 = "UPDATE asistencia SET irefrigerio='$HoraIni',frefrigerio='$HoraFin', Estado='FIN_REFRIGERIO',LongRefini='$longitud', LatRefini='$latitud', LongReffinal='$longitud', LatReffinal='$latitud' WHERE fregistro = '$freg' AND user = '$email' ";
                        ...
                        //Query grabar
                        $sql3 = "UPDATE asistencia SET irefrigerio='$Hora', Estado='INICIO_REFRIGERIO', LongRefini='$longitud', LatRefini='$latitud' WHERE fregistro = '$freg' AND user = '$email' ";
                        mysqli_query($conection, $sql3);
                        mysqli_close($conection);
                        $tipo_input = 'hidden';
                        
                        $alert = "Es hora de su refrigerio. Buen provecho!";
                        }
                }
            }else{
                    $alert ="Importante!. Inicio de refrigerio a partir de las 11:00 am";
            }
        }else{
            session_destroy();
...
" type="text/javascript"></script>
	<section id="container">
		...
		<form action="" method="post">
           ...
           <input type="<?php echo $tipo_input; ?>" name="Registrar" value="Inicio Refrigerio" style="font-size: 15px" onClick=""></input>
