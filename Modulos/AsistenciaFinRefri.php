    ...
    if($sessionTTL > $inactividad){
        ...
        if(!empty($email)){  
            if (!empty($_POST)){
                ...
                $consulta = "SELECT * FROM asistencia WHERE user = '$email' AND fregistro ='$freg' AND Estado='INICIO_REFRIGERIO'";
            ...
            if($mostrar3 > 0)  {
                            ...
                            //Query grabar
                            $sql3 = "UPDATE asistencia SET frefrigerio='$Hora', Estado='FIN_REFRIGERIO', LongReffinal='$longitud', LatReffinal='$latitud' WHERE fregistro = '$freg' AND user = '$email' ";
                            mysqli_query($conection, $sql3);
                            mysqli_close($conection);
                            $tipo_input = 'hidden';
                            $alert = "Es hora de continuar con sus labores. Suerte!";
            } 
            }
        }else{
            session_destroy();
...
" type="text/javascript"></script>
	<section id="container">
		...
		<form action="" method="post">
           ...
           <input type="<?php echo $tipo_input; ?>" name="Registrar" value="Fin Refrigerio" style="font-size: 15px" onClick=""></input>
